...
def switch_dms_theme(theme_name: str) -> None:
    """Switch DMS theme."""
    success, current_theme = run_command(["dms", "ipc", "theme", "getMode"])

    # Check if DMS is running
    if not success or any(x in current_theme.lower() for x in ["no running", "error", "fatal"]):
        print("DMS: Not running, skipping theme switch")
        return

    if current_theme.strip() == theme_name:
        print(f"DMS: Already using {theme_name} theme")
        return

    success, result = run_command(["dms", "ipc", "theme", theme_name])
    if success and not any(x in result.lower() for x in ["error", "fatal", "no running"]):
        print(f"DMS: Switched to {result.strip()}")
    else:
        print(f"DMS: Failed - {result.strip()}", file=sys.stderr)
